...
export class DaoSshOracleDB implements IDaoInterface {
  ...
  async getRowsFromTable(
    ...
    (tableName: string,
     ...
     filteringFields: any) {
      ...
      const filteringFieldsKeys = [];
      const filteringFieldsValues = [];
      for (const filteringField of filteringFields) {
        const { field, value } = filteringField;
        filteringFieldsKeys.push(field);
        filteringFieldsValues.push(value);
      }
      ...
      const result = await knex.transaction(trx => {
        knex.raw(`WITH RECORDSET AS (
          ...
          `, [...availableFields, ...searchedFieldsKeyValArr, ...filteringFieldsKeys, ...filteringFieldsValues, orderingField],
...
//***********************************************************************************
  ...
  async getTableStructure(tableName: string) {
    const schema = this.connection.schema ? this.connection.schema : (this.connection.username).toUpperCase();
    const knex = await this.createTunneledKnex();
    const structureColumns = await knex().select(
      'COLUMN_NAME',
      'DATA_DEFAULT',
      'DATA_TYPE',
      'NULLABLE',
  ...
  private static buildFilterWhere(filteredFields, where) {
